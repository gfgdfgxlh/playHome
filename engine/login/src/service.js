const engineRoot = ENGINE_PATH+'/login';
...
function service(net) {
  ...
  this.register = function(ctx){
    return ctx.render("login/web/register/index.ejs", {});
  }
  ...
  this.update = function(ctx, parms) {
    ...
    if (loginUser) {
      return ctx.render("login/web/update/index.ejs", {user:loginUser});
    } else {
      return ctx.render('login/web/update/back.ejs',{});
    }
  }
